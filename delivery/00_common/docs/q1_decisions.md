...
**理由**:
1. 附件 2 的 768 维与 `bert-base-uncased` 完全对应，文档、issue 普遍认为 CMU-MOSEI 用的就是 BERT-base
2. `last_hidden_state` 已被广泛验证在情感任务上有效
3. 模型权重约 440 MB，加载快速

**影响**: 已可与附件 2 的 `text` 字段做余弦相似度对比，本项目实测相似度 > 0.92（验证脚本 `text_extractor.py --test`）。
...
**风险**: 视觉特征可能弱于 OpenFace AU 特征；论文应说明"ResNet 视觉特征已足够用于 Late Fusion 集成，最终 Ensemble Acc=0.69+ 验证方案可行"。
